...
## Grid Shift Safety Margin (`shift_margin`)

Due to mechanical inaccuracies in stage movements, simply shifting by the overlap size might still result in exposure overlap between adjacent tiles on successive passes. To prevent this, a user-configurable **Grid Shift Safety Margin** (`shift_margin`) was introduced.

- **Calculation**: The margin is added to both the X and Y axes of the physical grid shift vector. This ensures the shifted footprint completely clears the previous footprint's boundary.
- **Unified Offset**: The margin is combined with the `row_shift` (Alternating row shift) displacement to produce the final `local_dx` and `local_dy` base pixel shifts before rotation.
- **Viewport Reflection**: The dashed grid boundary drawn in the main viewport dynamically expands to include this margin across all passes, giving the user an accurate visual representation of the maximum roaming area.

## User Interface Reorganization

To provide a cohesive user experience, all parameters dictating grid translation and staggered layouts were centralized in the Grid Settings Dialog. The following controls were extracted from disparate groups (Geometry and Advanced Settings) and consolidated into a dedicated **Grid shifting** `QGroupBox`:
1. **Alternating row shift in pixels**: Defines staggered tile layouts within the grid.
2. **Grid shift safety margin [px]**: Adds extra physical clearance between multi-slice passes.
3. **Apply automated slice shift**: Toggles the multi-slice grid translation feature.
4. **Passes**: Determines the loop frequency (N passes) before the shift resets.
