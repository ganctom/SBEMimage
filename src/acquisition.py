...
# ====================== STACK ACQUISITION THREAD run() ========================
    ...
    def run_acquisition(self):
        ...
        # Binary image masks for image quality inspection
    ...
    def acquire_all_grids(self):
        ...
        # Compute dynamic grid wd-adjustments
        if self.gm.grid_shift_active:
            if self.gm.wd_adjuster_obsolete():
                self.gm.init_focal_plane_processor()
            ...
            if self.gm[grid_index].slice_active(self.slice_counter):
                ...
                if (num_active_tiles > 0
                    ...
                    if grid_index in self.grids_acquired:
                        ...
                        # Perform optional dynamic grid procedures and acquire grid
                        try:
                            self.gm.prepare_dynamic_grid(grid_index)
                        except GridProcessingError as exc:
                            self.error_state = exc.code
                            utils.log_error(f"Dynamic‐grid setup failed: {exc}")
                        ...
                        try:
                            ...
                            self.error_state = exc.code
                            utils.log_error(f"Dynamic‐grid finalization failed: {exc}")
        ...
        if self.error_state == Error.none and not tile_skipped:
            ...
            if os.path.isfile(save_path):
                ...
                if not load_error:
                    ...
                    af = self.autofocus
                    is_ref = tile_index in af.afss_data['ref_tiles']
                    if tile_accepted and af.afss_active and is_ref:
