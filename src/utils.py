...
import warnings
warnings.filterwarnings('ignore', message='.*pillow version.*')
...
def load_masks(path):
    ...
    for fn in glob.glob(path + '\\mask_*.tif'):
        key = str.split(os.path.basename(fn), '.')[0]
        masks[key] = imread(fn)
    return masks


def load_image_collection(filenames: list) -> np.ndarray:
    # Use cv2 instead of skimage's ImageCollection to bypass libtiff C-level warnings
    # caused by proprietary Zeiss metadata tags in the SEM TIFF files.
    return np.array([cv2.imread(f, cv2.IMREAD_UNCHANGED) for f in filenames])
